...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT * FROM items WHERE Availability = 'YES' AND (ItemName LIKE '%$searchTerm%' OR Classification LIKE '%$searchTerm%' OR UserID LIKE '%$searchTerm%' OR NameOfUser LIKE '%$searchTerm%');";
    $result = mysqli_query($con, $sql);
    $resultCheck = mysqli_num_rows($result);
} else {
    // If no search term, execute the regular query
    $sql = "SELECT * FROM items WHERE Availability = 'YES' LIMIT $startFrom, $resultsPerPage;";
    $result = mysqli_query($con, $sql);
    $resultCheck = mysqli_num_rows($result);
}

$totalRows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM items WHERE Availability = 'YES';"));
?>
<html lang="en">
<head>
    <meta charset="UTF-8">
                    ...
                    <?php
                        if ($resultCheck > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                <td>' . $NameOfUser . '</td>
                                <td>
                                    <button class="btn btn-primary"><a href="EditBorrowx.php?updateItem=' . $itemNum . '" class="text-light">Edit</a></button>
                                </td>
                            </tr>';
                            }
                        } else {
                            echo '<tr><td colspan="7">All Items Are Being Borrowed.</td></tr>';
                        }
                    ?>
                </tbody>
            </table>
