...
if (isset($_POST['submit'])) {
    ...
    if ($newAvailability == $Availability && $newUserID == $UserID && $newNameOfUser == $NameOfUser) {
        // If no changes, redirect to Returnx.php
        header('location: Returnx.php');
        exit();
    }

    $sql = "UPDATE items SET Availability = '$newAvailability', UserID = ?, NameOfUser = '$newNameOfUser' WHERE itemNum = $itemNum";
    $stmt = mysqli_prepare($con, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "s", $newUserID);
...
<body>
    ...
    <div class="header">
        <div class="side-nav">
            ...
            <ul class="nav-links">
                <li><a href="Usersx.php"><i class="fa-solid fa-user"></i><p>USERS</p></a></li>
