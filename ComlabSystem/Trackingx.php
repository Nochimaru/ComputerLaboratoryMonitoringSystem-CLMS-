<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Comlab Inventory System</title>
    <link rel="stylesheet" type="text/css" href="styleHomex.css">
    <script src="https://kit.fontawesome.com/c4254e24a8.js" crossorigin="anonymous"></script>

</head>
<body>
<div class="parent">
        <div class="childleft">
            <a href="itemTrackingx.php"><img src="DeviceList.png" class="logo-img"></a>

        </div>
    </div>
    <div class="parent1">
        <div class="childleft">
            <a href="Borrowx.php"><img src="BorrowDevice.png" class="logo-img"></a>

        </div>
        <div class="childright">
            <a href="Returnx.php"><img src="ReturnDevice.png" class="logo-img"></a>

        </div>
    </div>

    <div class="header">
        <div class="side-nav">z
            <a href="Homex.php" class="logo">
            <img src="logo.png" class="logo-img">
            <img src="logo2.png" class="logo-icon">
            </a>
            <ul class="nav-links">
                <li><a href="Tracking.php"><i class="fa-solid fa-eye"></i><p>ITEM TRACKING</p></a></li>
                <li><a href="Reportx.php"><i class="fa-solid fa-chart-simple"></i><p>REPORTS</p></a></li>
                <li><a href="logout.php"><i class="fa-solid fa-arrow-left"></i><p>LOGOUT</p></a></li>
                <div class = "active"></div>
            </ul>
        </div>
    </div>


</body>

</html>